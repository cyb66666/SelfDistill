...
class VisionTransformer(nn.Module):
    ...
    def teacher_roi_encode(
        ...
        """
        batch_size, max_boxes = x.shape[:2]
    ...
    def student_encode(
        ...
        # ==================== 2. 处理 boxes ====================
        batch_size, max_boxes, _ = normed_boxes.shape
    ...
    def _attn_pool(self, roi_feat, teacher_cls_features):
        """
        Attention pooling using teacher CLS as query

        Args:
            roi_feat: [N, T, dim]
            teacher_cls_features: [N, dim]

        Returns:
            pooled: [N, dim]
        """

        query = teacher_cls_features.unsqueeze(1)  # [N,1,dim]
    ...
    def encode_dense(self, x, keep_shape=False, mode='maskclip'):
        # 1. Patch Embedding：将图像切分为patch并映射到特征维度
        x = self.conv1(x)  # shape = [*, width, grid_h, grid_w]
        # 示例：输入(bs,3,224,224) + ViT-B-32 → 输出(bs,768,7,7)（grid=7=224/32）
        bs, _, h, w = x.shape  # bs=批次大小，h/w=patch网格的高/宽（如7x7）
        
        # 2. 维度重排：从 [bs, width, h, w] → [bs, h*w, width]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # [bs, width, h*w]
        x = x.permute(0, 2, 1)  # [bs, h*w, width]（将patch序列维度前置）
        
        # 3. 拼接CLS Token：兼容ViT的标准输入格式
        x = torch.cat(
            [self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device),
            x], dim=1)  # shape = [bs, h*w + 1, width]
        # 注：CLS Token是ViT的全局特征token，这里保留以兼容Transformer编码逻辑
        
        # 4. 位置编码：适配输入尺寸（支持非标准尺寸图像）
        if (h, w) == self.grid_size:  # 输入尺寸与模型预设grid_size一致（如224→7x7）
            pe = self.positional_embedding.to(x.dtype)  # 使用原始位置编码
        else:  # 输入尺寸不一致（如384→12x12），插值缩放位置编码
            pe = self.rescale_positional_embedding(out_size=(h, w), dtype=x.dtype)
        x = x + pe  # 位置编码叠加到特征上（ViT核心操作）
        
        # 5. Patch Dropout：可选的patch级随机丢弃（正则化）
        # patch_dropout=0时为Identity，无实际作用
        x = self.patch_dropout(x)
        
        # 6. 前置LayerNorm：标准化输入特征
        x = self.ln_pre(x)
        
        # 7. 维度重排：适配Transformer输入格式（NLD → LND）
        # NLD: [bs, seq_len, width] → LND: [seq_len, bs, width]（Transformer标准输入）
        x = x.permute(1, 0, 2)
        
        # 8. Transformer编码（核心：不同注意力模式分支）
        if 'distill' in mode:  # 蒸馏模式：返回主特征+额外蒸馏特征
            x, extra_feats = self.transformer.extract_feature_map(x, mode=mode)
        else:  # 普通模式：仅返回主特征
            x = self.transformer.extract_feature_map(x, mode=mode)
        # extract_feature_map逻辑：
        # - 前layers-1层正常编码
        # - 最后一层按mode选择注意力分支（maskclip/QQ/KK/CSA）
        
        # 9. 维度还原：LND → NLD
        x = x.permute(1, 0, 2)  # [seq_len, bs, width] → [bs, seq_len, width]
        
        # 10. 池化处理：移除CLS Token，保留patch特征（密集特征核心）
        if self.attn_pool is not None:  # 若使用注意力池化
            x = self.attn_pool(x)
            x = self.ln_post(x)
            _, tokens = self._global_pool(x)  # 丢弃CLS，保留patch tokens
        else:  # 普通全局池化
            tokens = self.ln_post(x)  # 后置LayerNorm
        
        # 11. 投影到输出维度（如512）
        if self.proj is not None:
            tokens = tokens @ self.proj  # [bs, h*w, width] → [bs, h*w, output_dim]
        
        # 12. 构建密集特征图
        feature_map = tokens.view(bs, h * w, -1)  # 确保形状为[bs, h*w, dim]
        feature_map = F.normalize(feature_map, dim=-1)  # L2归一化（CLIP标准操作）
        
        # 13. 可选：还原空间形状（保留h/w维度）
        if keep_shape:
            # 从[bs, h*w, dim] → [bs, h, w, dim] → [bs, dim, h, w]（符合CNN特征图格式）
            feature_map = feature_map.view(bs, h, w, -1).permute(0, 3, 1, 2)
        
        # 14. 返回结果（兼容蒸馏/普通模式）
        if 'distill' in mode:
            return feature_map  # 蒸馏模式语义特征图+视觉特征图
        else:
            return feature_map  # 普通模式仅返回语义特征图
